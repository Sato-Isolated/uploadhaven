...
UploadHaven uses custom React hooks to extract business logic from components, following the
**Single Responsibility Principle** and enabling better code reuse and testing.
...
```typescript
const { data, fetchNextPage, hasNextPage, isLoading } = useActivitiesQuery();
```
...
```typescript
const { uploading, uploadFile, deleteFile, validateFile } = useFileOperations();
```
...
```typescript
const { isScanning, scanProgress, scanResults, startScan, stopScan } = useSecurityScanning();
```
...
```typescript
const { selectedFiles, toggleFileSelection, bulkDelete, exportData } = useAdminFileManager(files);
```
...
```typescript
export function useWithErrorHandling<T, P>(operation: (params: P) => Promise<T>) {
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);
...
export function useTypedHook<T extends BaseType>(params: HookParams<T>): HookReturn<T> {
  // Implementation
}
```
