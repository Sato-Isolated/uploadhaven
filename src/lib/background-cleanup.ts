...
class BackgroundCleanupService {
  ...
  async start(): Promise<void> {
    ...
    console.log(`Background cleanup service started (interval: ${this.intervalMs / 60000} minutes)`)
  }
  ...
  async runCleanup(): Promise<CleanupStats> {
    try {
      ...
      console.log(`Background cleanup: Found ${expiredFiles.length} expired files`)
      ...
      for (const file of expiredFiles) {
        try {
          ...
          try {
            ...
            console.warn(`Background cleanup: Could not delete physical file ${file.filename}:`, (fsError as Error).message)
          }
      ...
      console.log(`Background cleanup completed: ${stats.deletedCount}/${stats.totalExpired} files processed`)
    ...
    try {
      ...
      if (file.expiresAt && file.expiresAt < now) {
        ...
        try {
          const filePath = path.join(this.uploadsDir, file.filename)
          await unlink(filePath)
          console.log(`Instant deletion: Deleted expired file ${file.filename}`)
        } catch (fsError) {
          console.warn(`Instant deletion: Could not delete physical file ${file.filename}:`, (fsError as Error).message)
        }
  ...
  async checkForInstantExpiration(): Promise<CleanupStats> {
    try {
      ...
      return {
        deletedCount: 0,
        totalExpired: 0,
        errors: [`Instant expiration check failed: ${(error as Error).message}`]
      }
    }
  }
}
